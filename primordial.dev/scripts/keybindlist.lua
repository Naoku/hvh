    ...
    if keybind_enable:get() or entity_list.get_local_player() then 
        ...
        for i, v in pairs(keybindings) do
            ...
            if dap:get() or menu.is_open() then
                ...
                render.rect_filled(vec2_t(keybindlist_position.x-10, keybindlist_positiony2+offset*18+offset*1), vec2_t(195, 19), color_t(13,13,13,110))
                render.text(font, keybindlist_modes[dap:get_mode()+1], vec2_t(keybindlist_position.x + 175 - itssize.x, keybindlist_positiony2+18*offset+offset*1+2), color)
                render.text(font, i, vec2_t(keybindlist_position.x+2, keybindlist_positiony2+18*offset+offset*1+2),color)
                offset = offset + 1
